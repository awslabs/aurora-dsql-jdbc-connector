...
public final class TokenManager {
    ...
    private static final ConcurrentHashMap<TokenKey, CachedToken> TOKEN_CACHE = new ConcurrentHashMap<>();
     ...
     */
    @Nonnull
    public static String getToken(@Nonnull final String hostname, @Nonnull final Region region, @Nonnull final String user, 
                                 @Nonnull final AwsCredentialsProvider credentialsProvider, final Duration tokenDuration) throws SQLException {
        final TokenKey key = new TokenKey(hostname, region, user, credentialsProvider, tokenDuration);
     ...
     */
    @Nonnull
    public static String forceRefreshToken(@Nonnull final String hostname, @Nonnull final Region region, @Nonnull final String user, 
                                          @Nonnull final AwsCredentialsProvider credentialsProvider, final Duration tokenDuration) throws SQLException {
        final TokenKey key = new TokenKey(hostname, region, user, credentialsProvider, tokenDuration);
    ...
    private static String refreshToken(@Nonnull final TokenKey key) throws SQLException {
        try {
            LOGGER.fine(() -> "Aurora DSQL: Generating new authentication token for user: " + key.user);
            ...
            final Consumer<GenerateAuthTokenRequest.Builder> requester = builder -> {
                builder.hostname(key.hostname)
                        .region(key.region);
            ...
            final Duration cachedTokenDuration = key.tokenDuration == null ? Duration.ofMinutes(15) : key.tokenDuration;
            ...
            LOGGER.fine(() -> String.format("Aurora DSQL: Token generated successfully. Duration: %d seconds, Expires at: %s",
                    cachedTokenDuration.getSeconds(), cachedToken.expiresAt));
            
            return token;
            
        } catch (Exception e) {
            LOGGER.severe("Aurora DSQL: Failed to generate authentication token: " + e.getMessage());
    ...
    private static class TokenKey {
        ...
        TokenKey(String hostname, Region region, String user, 
                AwsCredentialsProvider credentialsProvider, Duration tokenDuration) {
    ...
    private static class CachedToken {
        ...
        boolean isExpiredOrExpiringSoon(double bufferPercentage) {
            ...
            final Duration bufferTime = totalLifetime.multipliedBy((long) (bufferPercentage * 100)).dividedBy(100);
